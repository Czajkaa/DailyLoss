...
class Food_Lunch : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_food_lunch)

        val buttonAdd: Button = findViewById(R.id.lunch_macro_button)
        sharedPreferences = getSharedPreferences("my_preferences", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()

        val product = Intent(this, Search_Product::class.java)
        buttonAdd.setOnClickListener {
            editor.putString("selected_time_to_eat", "Lunch")
            editor.apply()
        ...
        if (SharedPreferenceHelper2.isListExists(context)) {
            ...
            for (i in 1..productList.size) {
                ...
                val check = input.substring(0, 14)  // Set number for time to it (09_09_24_Lunch -> 14)
                if(selectedDate + "_" + "Lunch" == check) {
                    newProductView (input)
                }
            }
        }

        val allCalorie_value = findViewById<TextView>(R.id.lunch_macro_macroValue)
        val allProtein_value = findViewById<TextView>(R.id.lunch_macro_proteinValue)
        val allCarbs_value = findViewById<TextView>(R.id.lunch_macro_carbsValue)
        val allFats_value = findViewById<TextView>(R.id.lunch_macro_fatsValue)
        val allProtein_progress = findViewById<ProgressBar>(R.id.lunch_macro_protein_progressBar)
        val allCarbs_progress = findViewById<ProgressBar>(R.id.lunch_macro_carbs_progressBar)
        val allFats_progress = findViewById<ProgressBar>(R.id.lunch_macro_fats_progressBar)
        ...
        editor.putString(selectedDate + "Lunch" + "DayCalories", String.format(Locale.US, "%.1f", sum_calories))
        editor.putString(selectedDate + "Lunch" + "DayProtein", String.format(Locale.US, "%.1f", sum_protein))
        editor.putString(selectedDate + "Lunch" + "DayCarbs", String.format(Locale.US, "%.1f", sum_carbs))
        editor.putString(selectedDate + "Lunch" + "DayFats", String.format(Locale.US, "%.1f", sum_fats))
        editor.apply()
    }

    private fun newProductView (key: String) {
        val recyclerView: RecyclerView = findViewById(R.id.lunch_macro_recyclerView)
