...
class Food_Breakfast : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonAdd.setOnClickListener {
            editor.putString("selected_time_to_eat", "Breakfast")
            editor.apply()
        ...
        if (SharedPreferenceHelper2.isListExists(context)) {
            ...
            for (i in 1..productList.size) {
                ...
                val check = input.substring(0, 18)  // Set number for time to it (09_09_24_Breakfast -> 18)
                if(selectedDate + "_" + "Breakfast" == check) {
                    newProductView (input)
                }
            }
        }

        val allCalorie_value = findViewById<TextView>(R.id.breakfast_macro_macroValue)
        val allProtein_value = findViewById<TextView>(R.id.breakfast_macro_proteinValue)
        val allCarbs_value = findViewById<TextView>(R.id.breakfast_macro_carbsValue)
        val allFats_value = findViewById<TextView>(R.id.breakfast_macro_fatsValue)
        val allProtein_progress = findViewById<ProgressBar>(R.id.breakfast_macro_protein_progressBar)
        val allCarbs_progress = findViewById<ProgressBar>(R.id.breakfast_macro_carbs_progressBar)
        val allFats_progress = findViewById<ProgressBar>(R.id.breakfast_macro_fats_progressBar)
        ...
        editor.putString(selectedDate + "Breakfast" + "DayCalories", String.format(Locale.US, "%.1f", sum_calories))
        editor.putString(selectedDate + "Breakfast" + "DayProtein", String.format(Locale.US, "%.1f", sum_protein))
        editor.putString(selectedDate + "Breakfast" + "DayCarbs", String.format(Locale.US, "%.1f", sum_carbs))
        editor.putString(selectedDate + "Breakfast" + "DayFats", String.format(Locale.US, "%.1f", sum_fats))
        editor.apply()
    }
    ...
    private fun newProductView (key: String) {
        ...
        val calories = sharedPreferences.getString(string, "0.0").toString()
        string = key + "_" + "Protein"
        val protein = sharedPreferences.getString(string, "0.0").toString()
        string = key + "_" + "Carbs"
        val carbs = sharedPreferences.getString(string, "0.0").toString()
        string = key + "_" + "Fats"
        val fats = sharedPreferences.getString(string, "0.0").toString()
